...
def save_kernel_picture(data,name,image_name):
    ...
    #plt.axis('off')
    datainterval = np.ones(shape=(79,1,3))
    datainterval2 = np.ones(shape=(1, 7, 3))
    datainterval3 = np.ones(shape=(7, 7, 3))
    for i in range(100):
        datar = data[i, 0, :, :]
        #print datar
        minr =datar.min()
        ...
        datafinal=np.array([datar,datag,datab])
        datafinal1=datafinal.reshape(7, 7, 3)
        if i==0:
...
def demo(net, image_name):
    ...
    for k, v in net.params.items():
        ...
        if k=="rpn_conv1":
            print 'k,v', k,v
            #print v.blobs_
            print "kernel shape", v[0].data.shape
            #plt.figure()
            #plt.imshow(v[0].data[2,2,:,:], cmap='gray')
            #plt.axis('off')
            
            #np.set_printoptions(threshold='nan')
            print "weight=", net.params[k][0].data
            print "bias=", net.params[k][1].data
