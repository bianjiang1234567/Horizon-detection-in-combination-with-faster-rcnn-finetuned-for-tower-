...
def compute_horizon(slope_dist, offset_dist, caffe_sz, sz, crop_info, bin_edges):
  ...
  caffe_right = np.tan(slope)*caffe_sz[1]/2 + c#在224*224中 中心到直线距离和角度

  # scale back to cropped image
  c_left = caffe_left * (crop_sz[0] / caffe_sz[0])
  c_right = caffe_right * (crop_sz[0] / caffe_sz[0])#剪裁的时候较长边对称剪裁 按照短边长度
  ...
  print "center=",center
  crop_center = [np.dot(x_inds,[.5, .5])-center[0], center[1]-np.dot(y_inds,[.5, .5])]#原图像相对于剪裁后的中心的相对位置
