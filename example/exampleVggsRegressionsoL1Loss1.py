...
if __name__ == '__main__': 
  ...
  # load network
  deploy_file = '../models/regression/regularize_so_L1Loss_vggs/deploy.net'
  #model_file = '../models/regression/regularize_so_l2_vggs/regularize_so_l2.caffemodel'
  model_file = '../models/regression/regularize_so_L1Loss_vggs/snapshots3/solver_iter_16000.caffemodel'
  caffe.set_mode_cpu()
  ...
  result = net.forward(data=caffe_input, blobs=['prob_slope', 'prob_offset','slope_regress','offset_regress'])
  slope_dist = result['prob_slope'][0]
  offset_dist = result['prob_offset'][0]
  slope_regress = result['slope_regress'][0]#degrees
  offset_regress = result['offset_regress'][0]#224*224 distance
  ...
  left, right = compute_horizon(slope_dist, offset_dist, caffe_sz, sz, crop_info, bin_edges, slope_regress,offset_regress)
